...
La salida para InDesign se genera con `php artisan app:export-indesign-txt`. Consulta exclusivamente productos maestros activos, aprobados y sin revisión pendiente, y produce el TXT tabulado de 15 columnas **TAPA AMBA**. Categoría y grupo quedan vacíos por defecto —pueden incluirse con `--include-category-group`—, la imagen usa `.\imagenes\{CODIGO}.png`, y `--dry-run --json` permite revisar header y filas sin crear archivos. La exportación real se guarda en `storage/app/exports` y no se incorpora al repositorio; el formato simple `MARCA;Descripción catálogo` queda descartado.

`master_products` no almacena precios. Aporta código, marca y descripción homologadas, UXB, clasificación, referencia de imagen y demás datos maestros. `PRECIOLISTA`, `PRECIOOFERTA` y `PRECIOTACHADO` permanecen vacíos hasta integrar una fuente administrativa externa de pedido, promoción o lista, que se cruzará por `CODIGO`/SKU sin copiar precios al maestro.

La medida/presentación estable sí se guarda en el maestro. Si una aprobación no aporta `medida_catalogo`, queda marcada para revisión de medida sin bloquear la aprobación. Desde el detalle de Productos Maestros puede completarse manualmente —actualizando los textos comerciales y registrando logs— o declararse no aplicable mediante una excepción con motivo, usuario y fecha en `data.measurement`. InDesign omite y reporta productos sin medida ni excepción, y admite los que tengan una excepción explícita. El producto `30385` se corrige de forma controlada a `240GR`.
